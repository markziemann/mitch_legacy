...
```{r,geneset_quadrants,results="asis",echo=FALSE}
...
```{r,geneset_sizes,results="asis",echo=FALSE,fig.height = 10, fig.width = 7}
  par(mfrow=c(3,1))
...
```{r,tauchart1,results="asis",echo=F, fig.height = 7, fig.width = 7}
library("taucharts")
library(dplyr)
...
```
Now showing the top sets ranked by p.adjustMANOVA but without a cut-off.

```{r,tauchart2,results="asis",echo=F, fig.height = 7, fig.width = 7}
library("taucharts")
library(dplyr)
...
```{r,results_table,results="asis",echo=FALSE}
  ...
  kable( formatted , col.names = colnames(myres) , row.names=F, caption = "Top gene sets" ,digits=100)
  cat("<hr><br>")
```
...
```{r,detailed_geneset_reports1,results="asis",echo=FALSE,fig.height=5, fig.width=6,out.width = '40%',comment=NA}
...
for ( s in 1:resrows ) {
  ...
  size<-ll$setSize
  sss<-as.data.frame(res$detailed_sets[[i]])
#  sss<-as.data.frame(ss[rownames(ss) %in% res$input_genesets[res$input_genesets[,1]== ll$set,2],])
  ...
  print(kable( sss  , format="html",row.names=T, caption = "All member genes" ,digits=100))
