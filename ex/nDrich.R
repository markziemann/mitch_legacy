...
#    sss<-as.data.frame(ss[rownames(ss) %in% res$input_genesets[res$input_genesets[,1]== ll$set,2],])
  ...
  p<-ggpairs(as.data.frame(x), lower  = list(continuous = ggpairs_points_plot ))
  p +  theme_bw()
  ...
  #pairs contour plot
  p<-ggpairs(as.data.frame(ss), lower=list(continuous=ggpairs_func),
...
#   sss<-as.data.frame(ss[rownames(ss) %in% res$input_genesets[res$input_genesets[,1]== ll$set,2],])
    ...
    p<-ggpairs(as.data.frame(sss), lower=list(continuous=ggpairs_contour_limit_range),
      diag=list(continuous=wrap("barDiag", binwidth=nrow(ss)/10)) )
    print( p + theme_bw() )

    p<-ggpairs(as.data.frame(sss), lower= list(continuous = ggpairs_points_limit_range ),
          diag=list(continuous=wrap("barDiag", binwidth=nrow(ss)/10)))
    print( p + theme_bw() )
