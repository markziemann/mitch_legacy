...
if (d==1) {
  unformatted<-t(as.data.frame(res$analysis_metrics[ c(1,2,3,4,5 ) ])) 
  formatted<-unformatted
  formatted<-as.character(round(as.numeric(unformatted) , digits=0))
} else if (d==2) {
  ...
  unformatted<-t(as.data.frame(res$analysis_metrics[ c(2,3,4,5 )  ]))
  formatted<-as.character(round(as.numeric(unformatted) , digits=0))
}
...
```{r,input_geneset_metrics3,results="asis",echo=FALSE,message=FALSE}
if (d<2) {
...
cat(paste("Top N=",resrows,"sets irrespective of FDR. Try hovering over the points.<br>"))
# volcano tauchart
...
kable( formatted , col.names = colnames(formatted) , row.names=F, caption = cat(paste("Top N=",resrows,"gene sets")) ,digits=100)
cat("<hr><br>")
```
...
```{r,detailed_geneset_reports2d,results="asis",echo=FALSE,fig.height=5, fig.width=6 ,out.width = '80%',comment=NA, message=FALSE,eval=!D1}
cat("<h2> Detailed Gene set reports</h2><br>")
library("reshape2")
library("ggplot2")
library("GGally")
