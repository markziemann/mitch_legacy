...
```{r,heatmap,results="asis",echo=F, fig.height = 7, fig.width = 7 ,fig.show="all", message=FALSE}
if ( d==1 ) {
    message("Heatmap does not apply to unidimensional analysis.")
} else {
...
if ( d==1 ) {
    message("Does not apply to unidimensional analysis. See the volcano plot above")
} else {
  library(gplots)
...
if ( d==1 ) {
  resrows=length(res$detailed_sets)
  myres<-head(res$enrichment_result,resrows)
  myres[,c(3:6)]<-signif(myres[,c(3:6)],3)
} else if (d>1) {
  resrows=length(res$detailed_sets)
  myres<-head(res$enrichment_result,resrows)
...
if (d==1) { 
  ...
  for ( i in 1:resrows ) {
    ...
    print(kable(myres,format='markdown',caption=as.character(t(myres[i,1]))) )
    cat('\n')
    cat("<br>")
    ...
    colnames(tops)="Gene Rank"
    kable(head(tops,n=20L))
